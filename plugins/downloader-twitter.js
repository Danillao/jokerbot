...
const handler = async (m, {conn, text, usedPrefix, command}) => {
  if (!text) throw `*[❗𝐈𝐍𝐅𝐎❗]INSIRA UM LINK DO 𝚇 (𝚃𝚆𝙸𝚃𝚃𝙴𝚁), 𝙴X𝙴𝙼𝙿𝙻𝙾: ${usedPrefix + command}* https://twitter.com/auronplay/status/1586487664274206720?s=20&t=3snvkvwGUIez5iWYQAehpw`;
  try {
    const resFG = await twitter(text);
    await m.reply(global.wait);
    const captionFG = '*𝙰𝚀𝚄𝙸 𝙴𝚂𝚃𝙰 𝚂E𝚄 𝚅𝙸𝙳𝙴𝙾*';
    ...
    try {
      ...
      for (let x = 0; x < res.media.length; x++) {
        const caption = x === 0 ? res.caption.replace(/https:\/\/t.co\/[a-zA-Z0-9]+/gi, '').trim() : '*𝙰𝚀𝚄𝙸 𝙴𝚂𝚃𝙰 𝚂E𝚄 𝚅𝙸𝙳𝙴𝙾*';
        await conn.sendFile(m.chat, res.media[x].url, 'error.mp4', caption, m);
      }
    } catch {
      try {
        const AA = await savefrom(text);
        await conn.sendFile(m.chat, AA.url[0].url, 'error.mp4', '*𝙰𝚀𝚄𝙸 𝙴𝚂𝚃𝙰 𝚂E𝚄 𝚅𝙸𝙳𝙴𝙾*', m);
      } catch {
        throw '*[❗𝐈𝐍𝐅𝐎❗] 𝙴𝚁𝚁𝙾𝚁, 𝙿𝙾𝚁 𝙵𝙰𝚅𝙾𝚁 TENTE NOVAMENTE MAIS TARDE*';
      }
    }
  }
};
