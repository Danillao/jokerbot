...
—◉ Usuario: ${userr}
${prem ? `*◉ Tempo restante:*\n${clockString(usuario - new Date() * 1)}` : (isPrems ? `*◉ Tempo restante:*\n- Você é um usuario premium por tempo ilimitado` : '- Este é um usuario não premium ❌')}

*「 𝐔𝐒𝐔𝐀𝐑𝐈𝐎𝐒 𝐏𝐑𝐄𝐌𝐈𝐔𝐌 」*${sortedP.slice(0, len).map(({jid, name, premiumTime, prem, registered}, i) => `

—◉ Usuario: ${'@' + jid.split`@`[0]}
${premiumTime > 0 ? `*◉ Tempo restante:*\n${clockString(premiumTime - new Date() * 1)}` : '- Este é um usuario não premium ❌'}`).join('')}`.trim();

  if (sortedP.filter((user) => user.premiumTime).length === 0) {
    infoprem = `*「 𝐈𝐍𝐅𝐎 𝐃𝐎 𝐔𝐒𝐔𝐀𝐑𝐈𝐎 」*\n\n—◉ Usuario: ${userr}\n${prem ? `*◉ Tempo restante:*\n${clockString(usuario - new Date() * 1)}` : '- Você não é um usuario premium ❌'}\n\n*「 𝐔𝐒𝐔𝐀𝐑𝐈𝐎𝐒 𝐏𝐑𝐄𝐌𝐈𝐔𝐌 」*\n\n- Não tem usuarios premium ❌`.trim();
  }
